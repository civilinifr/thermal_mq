"""
This code fine-tunes the detections. Be sure to run the code "extract_evids" beforehand!

Now creates a fine-tuned catalog with information for each evid including:
- Absolute time, max amplitude, maxamp time, two kinds of SNR, and estimates the category of thermal moonquake
"""
# import packages
import pickle
import glob
...
def f_sta_lta2(t, f, Sxx, avg_Sxx, rel_detection, win, time, data, in_geophone_name, evid, specmax=1e-6):
    ...
    # mia_values = np.where(abs(data) < 10e-10)[0]
    # if len(mia_values) > 0:
    #     data[mia_values] = np.nan
    # Show functionally nan plots
    # fig = plt.figure(figsize=(10, 6))
    # ax = plt.subplot2grid((4, 1), (0, 0), 1, 1)
    # ax.plot(time, data)
    # ax.set_xlim((time[0], time[-1]))
    # plt.title(f'{in_geophone_name}, evid={evid}')
    # ax2 = plt.subplot2grid((4, 1), (1, 0), 3, 1)
    # ax2.scatter(time, data, s=0.3)
    # ax2.set_yscale('log')
    # ax2.set_xlim((time[0], time[-1]))
    # fig.tight_layout()
    # illustrator_output = '/home/francesco/Dropbox/synced_documents/publications/thermal_location/illustrator_edits/missing_data_display/'
    # plt.savefig(f'{illustrator_output}mia_data.png')
    # plt.savefig(f'{illustrator_output}mia_data.eps')
    ...
    if len(np.where(repeating_negative < new_arrival_index)[0]) > 0:
        # ###DEBUGGG CHECK THAT THIS WORKS IN PRACTICE!!!!!! ##############
    ...
    # #######DEBUG##########################
    # REMOVING DOUBLETS HERE
    # Edge case to remove the contribution of doublets.
    # Remove the repeating_negative_cut indices which occur AFTER another round of positives
    # doublet_negative_indices
...
# Input diretory where data should be processed
indir = 'D:/data_dir/'

# Original Civilini 2021 catalog
original_cat = 'C:/Users/Francesco/Dropbox/NASA_codes/publication/fc_lspe_dl_catalog.csv'
cat = pd.read_csv(original_cat)

# Input directory of combined_files created by "extract_evids"
input_directory = f'{indir}extract_evid/combined_files/'

output_directory = f'{indir}/fine-tuned/'
if not os.path.exists(output_directory):
    os.mkdir(output_directory)
cat_stats_dir = f'{indir}fine-tuned/cat_stats/'
...
mov_med_win_val = 5
# Gradient moving averaage
grad_mov_avg_val = 3
# Gradient-to-binary multiple
mult_grad_bin_val = 2
# Gap fill-time value
