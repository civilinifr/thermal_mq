"""
Plots examples of thermal moonquakes detected by the catalog

Be sure to run this before running fine-tune detection
...
def plot_individual_mq(input_file, evid, window_size=300):
    """
    ...
    # Plot the figure - Uncomment if you want to plot each individual moonquake
    ...
    # # plt.xlim((0, max(tme)))
    # plt.xlim((rel_det - window_size, rel_det + window_size))
    ...
    # # plt.title(f'Background val = {Sxx_background_val}')
    #
    # ax2 = fig.add_subplot(2, 1, 2)
    # specmax = 1e-6
    # plt.pcolormesh(t, f, Sxx, cmap=cm.jet, vmax=specmax, shading='auto')
    ...
    # plt.savefig(f"{out_image_directory}{evid_header['station']}/{plt_filename}.png")
...
def plot_mq_wrapper(input_info, pre_event_window_size=180, post_event_window_size=360):
    """
    Wrapper code for plotting the thermal moonquakes and saving the data in a pickle file

    :param input_info: [series] Displays the catalog information for the moonquake
    :param pre_event_window_size: [float] Time before event to begin cut (s)
    :param post_event_window_size: [float] Time after the event to end cut (s)
    :return:
    """
    ...
    data_files = sorted(glob.glob(f'{data_directory}{year}{month}{day}/{year}{month}{day}_17Geo*_{hour}_ID'))
    ...
    # We have to wait to initialize the arrays after getting the shape from the first piece of data
    geophone_ind = 0
    mia_vector = []
...
# Original Civilini 2021 Catalog (included here for ease)
catalog_file = 'C:/Users/Francesco/Dropbox/NASA_codes/publication/fc_lspe_dl_catalog.csv'

# Directory where the sac data is located
data_directory = 'D:/LSPE_sac/'

# Set the output directory
out_image_directory = 'D:/extract_evid/'
...
# Select all grade A events from Geophone 1 (Grade A =  seen on all 4 geophones)
...
# For largest events
# maxval_indices = find_largest_events(grade_a, 100)
# subset_cat = grade_a.iloc[maxval_indices]

# For random events
# num_events = 100
# np.random.seed(1)
# subset_indices = np.random.random_integers(0, high=len(grade_a), size=num_events)
# subset_cat = grade_a.iloc[subset_indices]

# For a particular event only
# subset_cat = subset_cat[subset_cat['evid'] == '761007-18-M1']
